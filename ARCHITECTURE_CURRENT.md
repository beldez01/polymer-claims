...
- **CES-0 → CES-4 (2026-06-10 → 12)** — the credibility-evidence spine: a content-addressed `AnalysisProfile` apparatus (CES-0) → a DRS-shaped SE-Contract data seam (CES-1) → the first claim to **license on a value computed from a methylation matrix** (CES-2, region Δβ, two methodologically-independent legs) → a license that records its **full content-address** — dataset `dimnames_hash` + apparatus `profile_hash` + `semantic_run_id` (CES-3) → all of it **running live** in `NodeRunner`/`serve` with a drift daemon that re-opens content-drifted licenses (CES-4). **EARNED (2026-06-17, Phase A):** the genome-wide **n-DMP count** now licenses at **REPRODUCED** on a **real TCGA-LAML HM450 cohort** (IDH-mut vs WT; 50,339 DMPs vs an 18,945 null floor; e-value → ∞; two independent legs agree; full content-address recorded). The recomputable-public tier is *earned, not just exercised*, for this reduction. Region-Δβ stays synthetic (its own real run pending); REPLICATED stays synthetic (needs a 2nd real cohort). Run caveats: IDH-mut n=10 (conservative — uncovered cases default WT, diluting the contrast); sex-chrom QC skipped (Xena betas carry no chr/pos); data local-only, gitignored.
