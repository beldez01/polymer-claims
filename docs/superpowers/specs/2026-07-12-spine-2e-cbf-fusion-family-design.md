...
generalizing the RUNX1-RUNX1T1 result. Two core-binding-factor (CBF) AML fusions, each aberrantly re-expressing
its normally-silent partner:

|  | RUNX1T1 | MYH11 |
|---|---|---|
| **t(8;21)+** | high (A, licensed) | silent |
| **inv(16)+** | silent | high |
...
- **B** — MYH11 clears the 13 TPM floor in `inv16` vs `other` (new re-expression floor).
- **C** — RUNX1T1 clears the floor in `t821` vs `inv16` (specificity — RUNX1T1 distinguishes the two CBF-AMLs).
- **D** — MYH11 clears the floor in `inv16` vs `t821` (specificity).

The specificity claims (C, D) are NOT a new claim type — they are the same floor+discrimination machinery with the
comparator `level_b` set to the OTHER fusion instead of `other`. Because each partner is silent outside its fusion,
the C/D discriminations are large.
...
- `se:tcga_laml_cbf_expr@1` and `se:target_aml_cbf_expr@1`: `Sample_Group ∈ {t821, inv16, other}` (3-valued;
  ...
  **{RUNX1T1 (ENSG00000079102), MYH11 (ENSG00000133392), ACTB, GAPDH}**. Fetched from the SAME already-downloaded
...
- Cohort counts (probed): TCGA t821≈6/inv16≈11; TARGET t821≈90/inv16≈112 (RNA-seq∩karyotype narrows these;
  the extract prints the real n per group). MYH11 = ENSG00000133392.
...
gene-tuned. The extract confirms each partner clears it in its fusion (RUNX1T1 in t821, MYH11 in inv16) as a
...
- `propose_cbf_family_claims(ref) -> list[Claim]`: builds A/B/C/D via `expression_floor_claim(gene=..., floor=13.0,
  ...
  `floor-MYH11-inv16-vs-other`, `floor-RUNX1T1-t821-vs-inv16`, `floor-MYH11-inv16-vs-t821`. Plus the ACTB control
...
- **Planted family licenses:** a synthetic 3-group contract (RUNX1T1 high only in t821, MYH11 high only in inv16,
...
1. TCGA + TARGET CBF contracts (`se:{cohort}_cbf_expr@1`) — controller-executed extracts (mirror 2d-i/2d-iii;
   3-valued labels + MYH11), self-checks per group.
