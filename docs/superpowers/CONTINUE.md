...
fields) nearly complete — #1 (provenance), #3 (governance), #4 (online-FDR), #6 (reinterpret) all
done, plus the `Claim.subject` slot; only #2 (oracle, blocked on Phase 8) and #5 (representation_revision
meta-tier) remain. 189 tests, green.**
...
## ▶ NEXT: Phase 8 (evaluator), OR the last two Phase-7 odds-and-ends

Phase 7 (protocol-imposed fields, unified spec §5) is nearly done: #1 provenance ✅ + #3 governance ✅ + #4 online-FDR ✅ + #6 `reinterpret` ✅ + the `subject` slot ✅. **Only two §5 items remain:** (2) oracle credibility-qualification object — **blocked**: binds to `operations`, which v1.3 dropped → naturally falls out of Phase 8; (5) `representation_revision` meta-tier (claims about the IR itself, gated conservatively) — its own phase, low urgency.

**The big next milestone is Phase 8 = the evaluator** — the thing that RUNS the grammar (executes a claim's licensing/satisfaction against materialized data, turning the static IR into something that actually verifies). It's the gateway to the **protocol runtime** (8-stage flywheel + 3 daemons) and the [[platform vision]] (users deploy agents on their own compute to elaborate the claims universe). It introduces `operations`/compute-graph, which unblocks #2 (oracle). Largest lift so far — wants its own brainstorm on scope (what "evaluate" means in v1.3: re-run the L2 `Satisfaction` against a `MaterializationContext`? a dispatch registry like v1.2's evaluator? air-gapped two-impl verification?). Then the protocol runtime + the deferred 3D topology viewer.
