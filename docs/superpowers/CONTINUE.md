...
## ▶ NEXT: oracle dossier (#2, now unblocked), the protocol runtime, OR Phase-7 #5

All 8 grammar layer-phases are done. The natural next moves, in rough priority:

1. **Oracle credibility-qualification object (unified spec §5 #2) — now UNBLOCKED.** Phase 8 introduced `operations` and left an `OperationNode.oracle_ref: str | None` slot. Build the dossier object (validation tier + applicability domain + propagated uncertainty), bind it to an operation node, and cap claim strength by oracle tier. Self-contained, high-leverage, small.
2. **The protocol runtime** — the 8-stage flywheel (REPRESENT→GENERATE→SAFETY-GATE→CANONICALIZE→SELECT→DESIGN→EXECUTE/GROUND→VERIFY→INTEGRATE) + 3 daemons, each a total `corpus_IR → corpus_IR`. Phase 8's `evaluate`/`verify` is the EXECUTE/GROUND + VERIFY core; the runtime wires the rest + the [[platform vision]] (users deploy agents on their own compute). Largest remaining lift; wants its own brainstorm/decomposition.
3. **`representation_revision` meta-tier (§5 #5)** — claims about the IR itself, gated conservatively. Its own phase, low urgency.
