...
fields) underway — online-FDR ledger (#4) shipped; the carried-forward `Claim.subject` slot also
shipped (biggest ingestion-fidelity gap, now closed). 178 tests, green.**
...
## ▶ NEXT: remaining Phase-7 protocol fields, OR Phase 8 (evaluator)

Phase 7 (protocol-imposed fields, unified spec §5) so far: #4 (online-FDR ledger) ✅ + #6 (`reinterpret` edge, in L3) ✅ + the carried-forward `subject` slot ✅. **Remaining §5 requirements** (each additive, no foundational change — pick one or bundle a coherent subset): (1) `generated_by` + `search_cardinality` provenance (a `Provenance` object on Claim — additive, clean, the obvious next); (3) `hazard_class` + governance/access-scope (a `Governance` object on Claim; activates the dormant `unreproducible_by_governance` status — additive, clean, load-bearing for TET2/TCGA); (2) oracle credibility-qualification object — **blocked**: binds to `operations`, which v1.3 dropped → belongs with the Phase-8 evaluator; (5) `representation_revision` meta-tier — conceptually heavy (claims about the IR itself), its own phase.
