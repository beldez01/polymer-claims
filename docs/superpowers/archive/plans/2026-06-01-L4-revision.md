...
- [x] Task 1 — partial entrenchment order — `f29c0b2`
- [x] Task 2 — entailment closure (Cn) — `e360816` + `4204541` (seeds-included assertion)
- [x] Task 3 — inconsistency detection — `7d79185` + `ac38fe4` (dedup comment)
- [x] Task 4 — RevisionResult + restore_consistency — `a758aea` + `7e73e37` (multi-conflict test, typed params, renamed extra-fields test). **Edge-hygiene fix baked in** (ultrathink audit, commit `8b3d00e`).
- [x] Task 5 — AGM expansion — `ea52857`
- [x] Task 6 — AGM contraction — `f21e5ba`
- [x] Task 7 — AGM revision (Levi identity) — `a05523a`
- [x] Task 8 — AGM postulate conformance tests + documented non-recovery — `cde0f89`
- [x] Task 9 — exports + whole-package verify — `1a2c86a`; polish `188cb50` (param annotations + status-asymmetry comment)
- [x] Final — merged `--no-ff` to main (`55096fd`), 147 tests green + ruff clean on merged result, branch deleted. Opus final review = READY TO MERGE (fuzzed 3000 graphs → core always consistent; entrenchment partial-order soundness exhaustively checked). Docs + memory updated.
