...
- [ ] **Step 1: Write the failing test** — APPEND; add `RetractionVerdict, RevisionResult, restore_consistency` to the top import from `polymer_grammar.revision`:
...
class RevisionResult(_Model):
    ...
    edges: tuple[DefeatEdge, ...]            # authored defeat edges carried through
...
def restore_consistency(
    ...
    return _result(new_claims, edges, verdict, prior_in)
```
...
Expected: PASS — all prior tests (117) + the new revision tests (28) = ~145 green; ruff clean. Confirm `tests/test_isolation.py` passes (no `polymer_formalclaim` import leaked in).
