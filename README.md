...
| `src/polymer_claims/` | **The umbrella distribution** (`polymer-claims`) — the CLI over the complete runtime + the live local node (`serve`, behind the optional `[serve]` extra). | **Active** — `pip install polymer-claims` → local node works. New CLI: `export-consistency` (sheaf gauge, `[embed]`), `ingest tcga-laml` + `serve --tcga-laml` (real TCGA-LAML n-DMP), `export-attestation` (in-toto/SLSA/DRS bundle + `--format dsse` DSSE NDJSON; arc 2 slices 1+2) |
